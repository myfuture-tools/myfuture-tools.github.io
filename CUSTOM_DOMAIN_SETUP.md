...
This guide walks through pointing a branded MyFuture subdomain at the GitHub Pages site so the live URL becomes `https://tools.myfuture.co.nz/` instead of `https://myfuture-tools.github.io/`.

The default `myfuture-tools.github.io/` URL will keep working and will redirect to the custom domain once it's live.

> **Note — the CNAME is pre-staged.** The `CNAME` file is already committed on the **`custom-domain`** branch (not `main`), so `main` stays green and no domain error shows until we're ready. On go-live day you either **merge `custom-domain` → `main`**, or simply enter the domain in Settings → Pages (Step 3), which writes the same file to `main`. Either produces the identical result.
...
| Before | After |
|---|---|
| `https://myfuture-tools.github.io/` | `https://tools.myfuture.co.nz/` |
...
Paths are identical either side of the cutover — only the hostname changes — because the site already serves at the root.
...
|---|---|---|---|
| 1. Add DNS record | Whoever manages `myfuture.co.nz` DNS (Lara's registrar or IT contact) | Registrar / DNS provider control panel | 5 min config + propagation wait |
| 2. Add custom domain (merge branch or repo settings) | NOW Group (Chris) | github.com → repo Settings → Pages | 2 min |
...
**Recommended order: do step 1 FIRST**, wait until DNS has propagated, then do step 2. GitHub validates DNS at the moment the custom domain lands — if DNS isn't ready, validation fails and you'll have to retry.
...
| Points to / Target / Value | `myfuture-tools.github.io.` *(include the trailing dot if the panel allows it)* |
...
- Don't point the CNAME at `myfuture-tools.github.io/…` with a path — DNS CNAMEs target hostnames only, never paths.
...
# Expected output: myfuture-tools.github.io.
```
...
**Option A (cleanest — use the pre-staged branch):** merge `custom-domain` → `main`. This puts the `CNAME` file (already containing `tools.myfuture.co.nz`) onto `main`, and Pages picks up the custom domain automatically.

**Option B (via UI):**
1. Open <https://github.com/myfuture-tools/myfuture-tools.github.io/settings/pages>
2. Under **Custom domain**, enter: `tools.myfuture.co.nz`
3. Click **Save**.

Either way, GitHub will:
- Run a DNS check. If DNS is correctly configured (step 2 confirmed it), this passes within seconds.
- Ensure a `CNAME` file at the repo root containing `tools.myfuture.co.nz` (this is how GH Pages remembers the custom domain across redeploys — don't delete it).
...
curl -I https://myfuture-tools.github.io/
...
| `tools.myfuture.co.nz` returns 404 even though DNS resolves | The `CNAME` file at the repo root was deleted, or has the wrong domain inside | Check the file exists at <https://github.com/myfuture-tools/myfuture-tools.github.io/blob/main/CNAME> and contains exactly `tools.myfuture.co.nz`. If missing/wrong, re-save the custom domain in Settings → Pages. |
...
1. **In GitHub:** Settings → Pages → clear the Custom domain field → Save. The site reverts to `myfuture-tools.github.io/` immediately. (If you merged the `custom-domain` branch, also remove the `CNAME` file from `main`.)
...
- Update the four `[TOOL LINK]` buttons inside the Gamma / PDF lead-magnet to use `tools.myfuture.co.nz/…` instead of the `myfuture-tools.github.io/…` URLs.
- Update the ScoreApp result-page links the same way.
- README link map still applies — only the hostname changes.
